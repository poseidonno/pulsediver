...
def search():
    ...
    #搜素速度慢，待优化（redis热点数据存储，自动摘要算法优化）
    ...
    if sorted_documents:
        # 从 DataFrame 中筛选所需的行
        mask = df_data['ID'].isin(sorted_documents)

        selected_data = df_data[mask][['ID', 'Title', 'URL']]
        # 重新索引以匹配sorted_documents中的顺序
        selected_data = selected_data.set_index('ID').reindex(sorted_documents).reset_index()

        results = selected_data.to_dict(orient='records')
    ...
    return render_template('search.html', doc_list=paginated_results_with_summaries, length=len(results),execution_times=execution_time, query_word=query, page=page)