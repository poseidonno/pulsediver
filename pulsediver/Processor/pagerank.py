...
class PageRank():
    ...
    def __init__(self, G, T=300, eps=1e-6, beta=0.85) -> None:
        self.G = G
        self.N = len(G)
        self.T = T
        self.eps = eps
    ...
    def computePR(self):
        ...
        for time in range(self.T):
            ...
            if np.linalg.norm(R_new - R) < self.eps:
                break
            R = R_new.copy()
        return np.around(R_new / np.sum(R_new), 5)  # 归一化处理，使得总和为 1
...
def save_pagerank_to_csv(csv_file, pagerank_values):
    data = pd.read_csv(csv_file)

    # 将计算得到的 PageRank 值追加为一个名为 'PR' 的新列
    data['PR'] = pagerank_values
...
if __name__ == "__main__":
    ...
    save_pagerank_to_csv(csv_file_path,pagerank_values)
